import { getCardsForestArr } from "./_sources.js";
...
export const settingsGameForest = (count, time) => {
  ...
  startBtn.onclick = () => {
    ...
    setTimeout(() => {
      ...
      const imageBgSrc = `url(img/card-bg-forest.jpg)`;
...
export const startGameForest = (gameBox, count, time, imageBgSrc) => {
  setTimeout(() => {
    gameSubtitle.textContent = 'Location: Dark Forest';
    const gameName = 'Dark Forest';
    const forestCardsArr = getCardsForestArr()
    startGame(gameName, forestCardsArr, gameBox, count, time, imageBgSrc)
  }, 200);
};
